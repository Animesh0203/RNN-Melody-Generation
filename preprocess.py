...
KERN_DATASET_PATH = ""
SAVE_DIR = "dataset"
SINGLE_FILE_DATASET = "file_dataset5"
MAPPING_PATH = "mapping5.json"
SEQUENCE_LENGTH = 64
...
def encode_song(song, time_step=0.25, transpose_to_C=False):
    """Converts a score into a time-series-like music representation. Each item in the encoded list represents 'min_duration'
    quarter lengths. The symbols used at each step are: integers for MIDI notes, 'r' for representing a rest, 'chord:X.Y.Z'
    for representing chords, and '_' for representing notes/rests that are carried over into a new time step.

    :param song (m21 stream): Piece to encode
    :param time_step (float): Duration of each time step in quarter length
    :param transpose_to_C (bool): Whether to transpose the song to C major/A minor
    :return:
    """
    if transpose_to_C:
        song = transpose(song)
    ...
    for event in song.flat.notesAndRests:
        ...
        # handle chords
            ...
            symbol = "chord:" + ".".join(map(str, pitches))

        # convert the note/rest/chord into time series notation
        steps = int(event.duration.quarterLength / time_step)
        for step in range(steps):

            # if it's the first time we see a note/rest/chord, let's encode it
...
def create_mapping(songs, mapping_path):
    ...
    songs = re.findall(r"[A-G][#b]?(?:maj|min|dim|aug)?[0-9]?|r|\d+|_", songs)
...
def convert_songs_to_int(songs):
    ...
    for symbol in songs:
        try:
            ...
            # If the symbol is not in the mappings, skip it
            pass
